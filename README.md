...
- 用 **17 个物料、4 个层级** 的真实 BOM，把「成品 → 组件 → 零件 → 原材料」的层级结构画清楚；
- 用 **16 个演示步骤、8 条部门泳道、2 块联动看板**，把**课本图 1-1** 的业务流程一环一环播出来；
- 用 **16 个真实单据**（销售订单、MPS 表、MRP 表、请购单、采购订单、入库单、出库单、委外单、结算单……）把每一步的输入输出落到实处；
...
| | 功能 | 说明 |
|---|---|---|
| 🌲 | **BOM 结构树** | SVG 自动布局的多级树，节点自下而上流向成品；鼠标悬停任意节点，**高亮它的「来源 → 去向」整条路径**，其余节点淡出 |
| 🏭 | **生产流程流水线** | 5 个阶段卡片演示「原材料采购 → 委外加工 → 零件加工 → 组件装配 → 成品总装」，每阶段的投入 / 产出物料一眼可读 |
| 📋 | **物料清单（BOM 展开）** | 17 个物料的层级、编码、类型、单位、单位用量、父件、直接子件全表展开 |
| 🎬 | **经营业务流程演示** | 16 步动画演示：客户 → 销售部 → 计划部（MPS / MRP / 计划下达）→ 生产车间 → 仓库 → 采购部 → 供应商 → 财务部 |
...
| ⌨️ | **全键盘操作** | 讲课不用摸鼠标：方向键翻页、空格播放、数字键跳步、`D` 同屏、`R` 重播 |
...
```text
┌────────────────────────────────────────────────────────────┐
│ ① BOM 结构树        悬停高亮整条来源/去向路径               │
├────────────────────────────────────────────────────────────┤
│ ② 一整套生产流程     5 阶段卡片：采购→委外→加工→装配→总装   │
├────────────────────────────────────────────────────────────┤
│ ③ 物料清单（BOM 展开）17 个物料 × 8 列全字段表             │
├────────────────────────────────────────────────────────────┤
│ ④ 企业经营业务流程演示 ★核心                              │
│    ├─ 16 步控制器（上一步/下一步/重播/自动播放/同屏/重置）  │
│    ├─ 部门泳道流程图（8 泳道 · 20 节点 · 23 条连线）         │
...
演示产品：**电子挂钟（成品 10000）**，订单量 100 个，共 **17 个物料 / 4 个层级**：

```mermaid
graph BT
    A12110["铝材 8g (B) 12110"] --> A12100["长针 (M) 12100"]
    A12210["铝材 6g (B) 12210"] --> A12200["短针 (M) 12200"]
    A12310["铝材 3g (B) 12310"] --> A12300["秒针 (M) 12300"]
    A12411["塑料 200g (B) 12411"] --> A12410["盘体 (S) 12410"]
    A12421["薄膜 0.5g (B) 12421"] --> A12420["字膜 (M) 12420"]
    A12410 --> A12400["盘面 (M) 12400"]
    A12420 --> A12400
    A12100 --> A12000["钟盘 (M) 12000"]
    A12200 --> A12000
    A12300 --> A12000
    A12400 --> A12000
    A13100["塑料 120g (B) 13100"] --> A13000["钟框 (S) 13000"]
    A11000["机芯 (B) 11000"] --> A10000["电子挂钟 (M) 10000"]
    A14000["电池 (B) 14000"] --> A10000
    A12000 --> A10000
    A13000 --> A10000
```
...
| 层级 | 编码 | 名称 | 类型 | 单位用量 | 父件 |
|:---:|---|---|:---:|---|---|
| 0 | 10000 | **电子挂钟** | 🔵 M | 1 个 | —（成品） |
| 1 | 11000 | 机芯 | 🟠 B | 1 个 | 电子挂钟 |
| 1 | 12000 | 钟盘 | 🔵 M | 1 个 | 电子挂钟 |
| 1 | 13000 | 钟框 | 🟢 S | 1 个 | 电子挂钟 |
| 1 | 14000 | 电池 | 🟠 B | 1 个 | 电子挂钟 |
| 2 | 12100 | 长针 | 🔵 M | 1 个 | 钟盘 |
| 2 | 12200 | 短针 | 🔵 M | 1 个 | 钟盘 |
| 2 | 12300 | 秒针 | 🔵 M | 1 个 | 钟盘 |
| 2 | 12400 | 盘面 | 🔵 M | 1 个 | 钟盘 |
...
| 3 | 12410 | 盘体 | 🟢 S | 1 个 | 盘面 |
| 3 | 12420 | 字膜 | 🔵 M | 1 个 | 盘面 |
| 4 | 12411 | 塑料 | 🟠 B | 200 g | 盘体 |
| 4 | 12421 | 薄膜 | 🟠 B | 0.5 g | 字膜 |
| 4 | 13100 | 塑料 | 🟠 B | 120 g | 钟框 |

> **统计**：1 个成品 · 7 个自制件(M) · 8 个采购件(B) · 2 个委外件(S) · 4 个层级
...
| 字段 | 含义 | 注意 |
|---|---|---|
| `code` | 物料编码 | 需唯一（作为节点 key） |
| `name` | 物料名称 | 显示用 |
| `type` | `M` 自制 / `B` 采购 / `S` 委外 | 决定配色与游戏答案 |
| `unit` | 单位（个 / g） | 影响数量显示 |
| `qty` | **单位用量** | 父件每产 1 个所需的数量 |
...
- **动画打断保护**：用**代号（`animGen`）**作废旧动画，避免快速点「上一步/下一步」时被打断的旧动画跑完又把库存变动多加一遍；
...
按 `F11` 全屏 + 按 `D` 进入同屏演示；页面字号会随画幅自动放大。若仍偏小，可用浏览器 `Ctrl` + `+` 缩放。
</details>
...
数据（BOM、单据号、客户名、金额）均为**教学示例**，不代表任何真实企业。
